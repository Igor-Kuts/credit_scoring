...
# Здесь мы будем использовать и тестировать новые
# сгенерированные признаки
path_input = './new_features_credit_card.csv'
path_output = str(input('Input path to save file: '))

def main(path_output):
    
    df = pd.read_csv(path_input)
    idxs = pgsql_guest.get_df_from_query("SELECT sk_id_curr, sk_id_prev FROM previous_application")
    idxs_ = pgsql_guest.get_df_from_query('SELECT sk_id_curr, target FROM application_train_test')
    df = df.merge(idxs, on='sk_id_prev', how='left')
    df = df.merge(idxs_, on='sk_id_curr', how='left')
    ...
    significant_features = [i for i, j in significant_features.items() if 1 in j.values()]

    # Выгрузим данные
    df[['sk_id_prev'] + significant_features].to_csv(path_output, index=False)
