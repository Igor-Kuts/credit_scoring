...
def main(path_output):
    ...
    idxs =  pgsql_guest.get_df_from_query('SELECT sk_id_prev FROM previous_application')
    df = pgsql_guest.get_df_from_query('SELECT * FROM installments_payments')
    df_ = idxs.merge(df, on='sk_id_prev', how='left')
    idxs_ = pgsql_guest.get_df_from_query('SELECT sk_id_curr, target FROM application_train_test')
    df = idxs_.merge(df_.groupby('sk_id_curr', as_index=False).max(), on='sk_id_curr', how='left')
